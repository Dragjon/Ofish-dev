...
def mvv_lva(board, move):
    ...
    if victim is not None and aggressor is not None:
        return piece_values_endgame[victim.piece_type] - piece_values_middlegame[aggressor.piece_type]
    return 0

def negamax_alpha_beta(board, depth, alpha, beta, color, nodes):
    if depth == 0 or board.is_game_over():
        return quiescence(board, alpha, beta, color, 2), nodes

    legal_moves = sorted(list(board.legal_moves), key=lambda move: mvv_lva(board, move), reverse=True)

    for move in legal_moves:
        board.push(move)
        nodes += 1
        
        # Introduce a quick static evaluation before going deeper
        static_eval = evaluate_static(board)
        if static_eval <= alpha:  # Prune if the position is likely bad
            board.pop()
            break

        value, nodes = negamax_alpha_beta(board, depth - 1, -beta, -alpha, -color, nodes)
        value = -value
        board.pop()
...
def evaluate_static(board):
    ...
    for square in chess.SQUARES:
        ...
        if piece is not None:
            color = board.piece_at(square).color
            piece_type = board.piece_at(square).piece_type
            eval += piece_values_static[piece_type] if color == chess.WHITE else -piece_values_static[piece_type]
    ...
    while True:
        ...
        if input_line == "uci":
           ...
           while depth <= calculateMaxDepth(board):
                ...
                best_move, score, nodes = get_best_move(board, depth)
                ...
                print(f"info depth {depth} score cp {score} nodes {nodes} pv {best_move}")
